...
const NavTabs: FunctionComponent<NavTabsProps> = ({
  ...
  const styleClass = useStyles();
    ...
    const CustomToggleInstitution = React.forwardRef(({ children, /* onClick */ }, ref) => (
        <a
            ...
            <Box fontFamily={secondaryFont} fontWeight={700} fontSize={14} className={`${styleClass.dBlock}`}>
                ...
                <ExpandMoreIcon className={`${styleClass.mt2} ${styleClass.outlineNone} ${styleClass.rotateClosed}`} fontSize='large' /* hidden={parseInt(this.props.woundInfo.WoundID) === 0} *//>
    ...
    let tabs = <>
            <Dropdown className={`${styleClass.dBlock} ${styleClass.dLgNone} ${styleClass.dropdownTabs}`}>
                <Dropdown.Toggle
                    id={"0"}
            ...
            <Tabs
                className={`${styleClass.navbarTabHeight} ${styleClass.dLgBlock} ${styleClass.bigIndicator}`}
                classes={{ indicator: styleClass.bigIndicator }}
                indicatorColor="primary"
                textColor="primary"
                /* onChange={(event, value) => this.handleChangeTabs(event, value)} */
                value={0}>
                <Tab label="Patient Detail" key={0} />
                <Tab className={`${styleClass.opacity50}`} label="Characterisation" key={1} />
                <Tab className={`${styleClass.opacity50}`} label="Report" key={2} />
            </Tabs>
        </>

    return (
        <>
            <div className={`${styleClass.borderNav} ${styleClass.navContainer} ${/* this.state.indexTabs !== 1 ? "" : */ "sticky-characterization"}`} id="nav-container">
                <div className={`${styleClass.dFlex}`}>
                    <ButtonBase
                        className={`${styleClass.pr3} ${styleClass.prLg0} ${styleClass.mb2} ${styleClass.outlineNone}`}
                        /* disableRipple
                        disableTouchRipple
                        focusRipple
                        variant="contained"
                        ...
                        {/* (this.state.scrolled ? (
                                ...
                                <>*/}
                                    ...
                                    <Box
                                        ...
                                        className={`${styleClass.textUppercase} ${styleClass.pr5} ${styleClass.dNone} d-md-block`}>
                                        Pacientes
                                    </Box>
                    </ButtonBase>
                    {tabs}
                    <div className={`${styleClass.editControls}`} /* hidden={this.state.indexTabs !== 1} */>
                        <IconButton color="primary"
                            className={`${styleClass.pt2} ${styleClass.outlineNone}`}
